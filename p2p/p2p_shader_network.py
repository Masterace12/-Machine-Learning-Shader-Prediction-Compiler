...
class CryptographicValidator:
    ...
    def generate_key_pair(self, peer_id: str) -> Tuple[bytes, bytes]:
        ...
        if not CRYPTO_AVAILABLE:
            ...
            fake_private = f"private_key_{peer_id}_{random.randint(1000, 9999)}".encode()
            fake_public = f"public_key_{peer_id}_{random.randint(1000, 9999)}".encode()
            self.key_pairs[peer_id] = (fake_public, fake_private)
            return fake_public, fake_private
            
        # Generate real RSA key pair
        private_key = rsa.generate_private_key(
            public_exponent=65537,
            key_size=2048  # Smaller key for Steam Deck performance
        )
        ...
        self.key_pairs[peer_id] = (public_pem, private_pem)
        return public_pem, private_pem
    
    def sign_data(self, peer_id: str, data: bytes) -> bytes:
        """Sign data with peer's private key"""
        if not CRYPTO_AVAILABLE or peer_id not in self.key_pairs:
            # Fallback to simple hash-based signature
            signature_data = f"{peer_id}:{hashlib.sha256(data).hexdigest()}"
            return signature_data.encode()
            
        # Real cryptographic signature
        _, private_key_pem = self.key_pairs[peer_id]
